#ifndef JIT_BACKEND_X64_WINDOWS_H
#define JIT_BACKEND_X64_WINDOWS_H


#include "JitCommon.h"

/* TODO: SSE only supports 8 regs, but AVX can do 16 */
#define REG_COUNT 8

typedef struct jit_backend_x64_windows
{
    /* program memory */
    u8 *Program; 
    uint ProgramSize; 
    uint ProgramCapacity;

    /* data memory */
    void *GlobalData;
    uint GlobalDataSize;
    uint GlobalDataCapacity;
    int DataSize;

    /* reg management */
    u8 BusyReg[REG_COUNT];
    uint BusyRegCount;
    /* stack management */
    int StackSize;
    int MaxStackSize;
    /* global management */

    u8 StoreSingle[3];
    u8 LoadSingle[3];
    u8 FloatOpcode;
} jit_backend;




typedef enum reg_index 
{
    RAX = 0, 
    RCX, 
    RDX, 
    RBX, 
    RSP, 
    RBP, 
    RSI, 
    RDI,
} reg_index; 


static inline bool8 Backend_x64_IsAVXSupported(void);


#if (defined(_MSC_VER) && !defined(__clang__) /* clang is real funny */)\
|| (defined(_WIN32) && defined(__TINYC__)) /* tcc for windows */

static inline void Backend_x64_GetCPUID(u32 Feature, u32 *OutEAX, u32 *OutEBX, u32 *OutECX, u32 *OutEDX)
{
    int Bitfield[4];
    __cpuid(Bitfield, Feature);
    *OutEAX = Bitfield[0];
    *OutEBX = Bitfield[1];
    *OutECX = Bitfield[2];
    *OutEDX = Bitfield[3];
}

#else /* non-msvc compiler targeting windows */
#   include <cpuid.h>

static inline void Backend_x64_GetCPUID(u32 Feature, u32 *OutEAX, u32 *OutEBX, u32 *OutECX, u32 *OutEDX)
{
    __get_cpuid(Feature, OutEAX, OutEBX, OutECX, OutEDX);
}

#endif /* (_MSC_VER && !__clang__) || (_WIN32 && __TINYC__) */


static inline bool8 Backend_x64_IsAVXSupported(void)
{
    u32 EAX, 
        EBX, 
        ECX = 0, 
        EDX;
    Backend_x64_GetCPUID(1, &EAX, &EBX, &ECX, &EDX);
    return (ECX & (1 << 20)) != 0;
}

/* ms x64 calling conv */
static inline int Backend_StackMaxAlignment(int StackSize)
{
    if (StackSize % 16)
        return ROUND_UP_TO_MULTIPLE(StackSize, 16);
    return StackSize;
}
static inline int Backend_StackMinAlignment(int StackSize)
{
    if (StackSize % 8)
        return ROUND_UP_TO_MULTIPLE(StackSize, 8);
    return StackSize;
}
static inline int Backend_GetShadowSpaceSize(void)
{
    return 32;
}
static inline int Backend_GetGlobalPtrReg(void)
{
    return RCX;
}
static inline int Backend_GetStackFrameReg(void)
{
    return RBP;
}
static inline int Backend_GetArgStackSize(int ArgCount, int DataSize)
{
    (void)DataSize; /* stack will always be aligned to at least 8 byte boundary */
    ASSERT(DataSize == sizeof(double) || DataSize == sizeof(float), "invalid data size");
    /* rcx as global ptr reg, but we won't store it on stack */
    int StackSize = (ArgCount + 1) * 8;
    if (StackSize < 32)
        return 32;
    return StackSize;
}
static inline int Backend_GetArgRegCount(void)
{
    /* rcx taken as global ptr reg */
    /* xmm1 = arg0  */
    /* xmm2 = arg1  */
    /* xmm3 = arg2  */
    /* rest are on stack */
    return 3;
}
static inline bool8 Backend_CallerShouldSave(int Reg)
{
    return IN_RANGE(0, Reg, REG_COUNT - 1);
}
static inline bool8 Backend_IsArgumentInReg(int ArgIndex)
{
    return ArgIndex < Backend_GetArgRegCount();
}
static inline jit_reg Backend_CallerSideArgReg(int ArgIndex)
{
    ASSERT(Backend_IsArgumentInReg(ArgIndex), "unreachable");
    return ArgIndex + 1;
}
static inline jit_mem Backend_CallerSideArgMem(int ArgIndex)
{
    ASSERT(!Backend_IsArgumentInReg(ArgIndex), "unreachable");
    return (jit_mem) {
        .BaseReg = RSP,
        .Offset = (ArgIndex + 1)*8, /* arg0 = global ptr (rcx) */
    };
}
static inline jit_mem Backend_CalleeSideParamMem(int ArgIndex)
{
    return (jit_mem) {
        .BaseReg = RBP,
        .Offset = 0x10 + (ArgIndex + 1)*8, /* arg0 = global ptr (rcx) */
    };
}




#endif /* JIT_BACKEND_X64_WINDOWS_H */

